...
from src.data.datasets import TASK_TO_NUM
...
combination_eval_metric = "task"

if combination_eval_metric == "task":
    num_eval_metric = len(TASK_TO_NUM.keys())
    eval_metric_matrix = np.zeros((num_eval_metric, num_eval_metric))
    decidability_matrix = np.zeros((num_eval_metric, num_eval_metric))
elif combination_eval_metric == "test-retest":
    num_eval_metric = 9
    eval_metric_matrix = np.zeros((num_eval_metric, num_eval_metric))
    decidability_matrix = np.zeros((num_eval_metric, num_eval_metric))

for i in range(num_eval_metric):
    for j in range(num_eval_metric):
        if combination_eval_metric == "task":
            is_enrol_task = df["nb_task"] == i
            is_auth_task = df["nb_task"] == j

            print(f"Using {NUM_TO_TASK[i]} for ENROLMENT and {NUM_TO_TASK[j]} for AUTHENTICATION:")

            enroll_idx = df.index[is_round_1 & is_session_1 & is_enrol_task]
            auth_idx = df.index[is_round_n & is_session_2 & is_auth_task]

        elif combination_eval_metric == "test-retest":
            is_enrol_round = df["nb_round"] == i + 1
            is_auth_round = df["nb_round"] == j + 1

            print(f"Using R{i + 1} for ENROLMENT and R{j + 1} for AUTHENTICATION:")

            enroll_idx = df.index[is_enrol_round & is_session_1 & is_task]
            auth_idx = df.index[is_auth_round & is_session_2 & is_task]
        ...
        if combination_eval_metric == "task":
            eval_metric_matrix[i][j] = float(f"{100 * eer:.2f}")
            decidability_matrix[i][j] = float(f"{d:.2f}")
        elif combination_eval_metric == "test-retest":
            eval_metric_matrix[i][j] = float(f"{100 * eer:.2f}")
            decidability_matrix[i][j] = float(f"{d:.2f}")
...
plt.subplots_adjust(left=0.090, right=0.979, top=0.975, bottom=0.064)
img = plt.imshow(eval_metric_matrix, cmap=('plasma' if combination_eval_metric == "task" else 'viridis'))
plt.colorbar()

# for i in range(eval_metric_matrix.shape[0]):
#     for j in range(eval_metric_matrix.shape[1]):
#         plt.text(j, i, eval_metric_matrix[i, j], ha='center', va='center', color='white')

norm = Normalize(vmin=eval_metric_matrix.min(), vmax=eval_metric_matrix.max())
for i in range(eval_metric_matrix.shape[0]):
    for j in range(eval_metric_matrix.shape[1]):
        color = img.cmap(norm(eval_metric_matrix[i, j]))
        # Determine if the background color is light or dark
        if np.mean(color[:3]) > 0.5:  # Using RGB values
            text_color = 'black'
        else:
            text_color = 'white'
        plt.text(j, i, f"{eval_metric_matrix[i, j]:.2f}",
                 ha='center', va='center', color=text_color)

if combination_eval_metric == "task":
    task_names = list(TASK_TO_NUM.keys())
    plt.xticks(ticks=range(len(task_names)), labels=task_names, rotation=90)
    plt.yticks(ticks=range(len(task_names)), labels=task_names)
elif combination_eval_metric == "test-retest":
    rounds = list(range(1, 10))
    plt.xticks(ticks=range(len(rounds)), labels=[f"R{r}" for r in rounds], rotation=90)
    plt.yticks(ticks=range(len(rounds)), labels=[f"R{r}" for r in rounds])

plt.xlabel('Authentication')
plt.ylabel('Enrollment')
plt.title('EER Matrix')
plt.show()

plt.figure(figsize=(7, 6))
plt.subplots_adjust(left=0.090, right=0.979, top=0.975, bottom=0.064)
plt.tight_layout()
img = plt.imshow(decidability_matrix, cmap=('managua' if combination_eval_metric == "task" else 'cividis'))
plt.colorbar()

# for i in range(decidability_matrix.shape[0]):
#     for j in range(decidability_matrix.shape[1]):
#         plt.text(j, i, decidability_matrix[i, j], ha='center', va='center', color='white')

norm = Normalize(vmin=decidability_matrix.min(), vmax=decidability_matrix.max())
for i in range(decidability_matrix.shape[0]):
    for j in range(decidability_matrix.shape[1]):
        color = img.cmap(norm(decidability_matrix[i, j]))
        # Determine if the background color is light or dark
        if np.mean(color[:3]) > 0.5:  # Using RGB values
            text_color = 'black'
        else:
            text_color = 'white'
        plt.text(j, i, f"{decidability_matrix[i, j]:.2f}",
                 ha='center', va='center', color=text_color)
