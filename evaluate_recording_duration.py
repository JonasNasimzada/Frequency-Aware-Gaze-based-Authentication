...
for dir in [Path("./embeddings/EKYT"), Path("./embeddings/ViT"), Path("./embeddings/ResNet101")]:
    print("Using model:", model_name)

    fold_names = [model_name + f"_f{fold}.csv" for fold in range(4)]
    files_dict = {
        k: [dir / "_".join([k, name]) for name in fold_names]
        # k: [embed_dir / "_".join([k, name]) for name in fold_names]
        for k in ("val", "test", "judo")
    }
    has_judo = files_dict["judo"][0].exists()

    duration_matrix = np.zeros((12, 2))

    for n_seq in range(1, 3):
        ...
        # duration_matrix[n_seq-1, 0] = int(n_seq)
        # duration_matrix[n_seq-1, 1] = float(eer)

        # eer_by_dir[dir.name] = duration_matrix

# print(eer_by_dir)

# plt.figure(figsize=(10, 6))
# for dir_name, eer_values in eer_by_dir.items():
#     plt.plot(range(1, 13), eer_values, 'o-', label=dir_name, linewidth=1.5, markersize=4)

# Create bar plot
# plt.plot(duration_matrix[:, 0], duration_matrix[:, 1], 'o-', linewidth=1.5, markersize=4)

# Customize plot
# plt.xlabel('Number of Sequences (n_seq)')
# plt.ylabel('Equal Error Rate (EER)')
# plt.grid(True, axis='y', linestyle='--', alpha=0.7)
# # plt.xticks(duration_matrix[:, 0])
